...
for ind_file=1:length(filelist)
    ...
    if en_CR_PAA
        R_off_CR_PAA_c_aux=[R_off_CR_PAA_c_aux R_off_CR_PAA_c];
    end
    ...
    if en_BW_CR_PAA
        R_off_BW_CR_PAA_c_aux=[R_off_BW_CR_PAA_c_aux R_off_BW_CR_PAA_c];
    end
...
R_off_CR_PAA_c = R_off_CR_PAA_c_aux ;
R_off_CR_c=R_off_CR_c_aux ;
R_off_BW_CR_PAA_c = R_off_BW_CR_PAA_c_aux ;
...
clear Nuser_beam_c_aux  Req_user_c_aux  R_off_CR_PAA_c_aux R_off_BW_CR_PAA_c_aux ...
     R_off_CR_c_aux R_off_MAP_c_aux R_off_BW_CR_c_aux R_off_BW_MAP_c_aux R_off_BW_POW_c_aux ...
    user_beams_c_aux 
...
tech_label={'SR-BF','SR-BW-BF','SR','MAP','SR-BW','BW-MAP','BW-POW'};
marker_v=['v','o','s','^','+','d','*','x','v','p','.'];
Ntech=length(tech_label);
c6 = categorical(tech_label);
c6 = reordercats(c6,tech_label);


sim_sol= find([en_CR_PAA en_BW_CR_PAA en_CR en_MAP en_BW_CR en_BW_MAP en_BW_POW]);


NQU_sims=cell(1,Ntech);
NQU_av=ones(1,Ntech)*NaN;

NU_sims=cell(1,Ntech);
NU_av=ones(1,Ntech)*NaN;

Offered_sims=cell(1,Ntech);
Offered_av=ones(1,Ntech)*NaN;

Min_user_sims=cell(1,Ntech);
Min_user_av=ones(1,Ntech)*NaN;


colors(1,:)=[91 181 172]/256;
colors(2,:)=[192 50 26]/256;
colors(3,:)=[84 123 180]/256;
colors(4,:)=[221 124 79]/256;
colors(5,:)=[108 97 175]/256;
colors(6,:)=[98 156 53]/256;
colors(7,:)=[111 111 111]/256;
colors(8,:)=[0 0 0]/256;
...
if en_CR_PAA
    NQU_sims{1}=cellfun(@(x,y) sum(NQU_fun(x,y)),Req_user_c,R_off_CR_PAA_c);
    NQU_av(1)=mean(NQU_sims{1});
end

if en_BW_CR_PAA
    NQU_sims{2}=cellfun(@(x,y) sum(NQU_fun(x,y)),Req_user_c,R_off_BW_CR_PAA_c);
...
for ind_tech=sim_sol
    ...
    plot(x/factor_amp,F ,'LineWidth',2,'Color',colors(ind_tech,:),'DisplayName',tech_label{ind_tech},...
        'Marker',marker_v(ind_tech),'MarkerIndices', 1+20:300:length(x) );
...
if en_CR_PAA
    NU_sims{1}=cellfun(@(x,y) sum(NU_fun(x,y)),Req_user_c,R_off_CR_PAA_c);
    NU_av(1)=mean(NU_sims{1});
end

if en_BW_CR_PAA
    NU_sims{2}=cellfun(@(x,y) sum(NU_fun(x,y)),Req_user_c,R_off_BW_CR_PAA_c);
...
for ind_tech=sim_sol
    ...
    plot(x/factor_amp,F ,'LineWidth',2,'Color',colors(ind_tech,:),'DisplayName',tech_label{ind_tech},...
        'Marker',marker_v(ind_tech),'MarkerIndices', 1+20:800:length(x) );
...
if en_CR_PAA
    Offered_sims{1}=cellfun(@(x) Offered_fun(x),R_off_CR_PAA_c);
    Offered_av(1)=mean(Offered_sims{1});
end

if en_BW_CR_PAA
    Offered_sims{2}=cellfun(@(x) Offered_fun(x),R_off_BW_CR_PAA_c);
...
pts=2000:1:6800;
figure,hold on
plot(total_band/1e6*[272*0.05 272*0.05],[0 1],'k','LineWidth',2,'DisplayName','T, satellite capacity')
for ind_tech=sim_sol
...
if en_CR_PAA
    Min_user_sims{1}=cellfun(@(x)Min_user_fun(x),R_off_CR_PAA_c);
    Min_user_av(1)=mean(Min_user_sims{1});
end

if en_BW_CR_PAA
    Min_user_sims{2}=cellfun(@(x)Min_user_fun(x),R_off_BW_CR_PAA_c);
...
if en_CR_PAA
    aux_Rates{1}=total_band/1e6*cell2mat(R_off_CR_PAA_c(:));
end

if en_BW_CR_PAA
    aux_Rates{2}=total_band/1e6*cell2mat(R_off_BW_CR_PAA_c(:));
end
...
for ind_sim=1:Nsims
    ...
    for i=1:K
        ...
        if en_CR_PAA
            Offer_beam{1}(ind_sim,i)= total_band/1e6*sum ( R_off_CR_PAA_c{ind_sim}(user_index));
        end

        if en_BW_CR_PAA
            Offer_beam{2}(ind_sim,i)= total_band/1e6*sum ( R_off_BW_CR_PAA_c{ind_sim}(user_index));
        end
...
tech_label2={'Offered: SR-BF', 'Offered: SR-BW-BF', 'Offered: SR', 'Offered: MAP','Offered: SR-BW','Offered: BW-MAP','Offered: BW-POW'};
...
%% Average requested and offerd traffic rates varies with time
...
for ind_sim=1:Nsims
    ...
    Req_b1 = 0;
    for i = 1:Nsims/h_total
        Req_b1 = Req_b1 + Req_b(h_total*(i-1)+1:h_total*i,:);       
    end
    Req_b2 = Req_b1/Nsims*h_total;

res=[];
for ind_tech=sim_sol
    res= [ res , sum(Offer_beam{ind_tech},2)];
end

res1 = 0;
for i = 1:Nsims/h_total
    res1 = res1 + res(h_total*(i-1)+1:h_total*i,:);
end
res2 = res1/Nsims*h_total;

t = hours(5):hours(1):hours(5+h_total-1);
off = sum(Req_b2,2);
F = [off,res2];

colors1 = colors([8,sim_sol],:);
figure
for i = 1:length([1,sim_sol])
plot(t,F(:,i),'LineWidth',2,'Color',colors1(i,:),...
        'Marker',marker_v(i),'DurationTickFormat','hh:mm' );
hold on
end
...
plot([5,5+h_total-1],total_band/1e6*[272*0.05 272*0.05],'k','LineWidth',2,'DisplayName','\it T_u')

% title([ Scenario_label_text ': Offered and Requested rates varies with time'])
...
T = table(NQU_av',NU_av',Offered_av',Min_user_av', ...
    ...
    'Min. User Rate [Mbps]'},'RowName',{'SR-BF','SR-BW-BF','SR','MAP','SR-BW','BW-MAP','BW-POW'});
disp(T)
